...
export function generateFallbackReport(answers: AssessmentAnswers, scores: ScoreResult) {
  return [
    "Executive Summary",
    `The assessment indicates an Operational Intelligence Index of ${scores.operationalIntelligenceIndex}/100. The main opportunity is to reduce recurring workflow drag before it becomes a larger cost center.`,
    "",
    "Friction Analysis",
    ...
    `The Structural Debt Score is ${scores.structuralDebtScore}/100 across ${answers.tools.length} selected tools. Mindful Tech should map the handoffs between these systems before recommending automation.`,
    "",
    "Founder Dependency Analysis",
    `The Founder Dependency Index is ${scores.founderDependencyIndex}/100. Leadership time appears to be absorbed by operational coordination that can likely be delegated, documented, or systemized.`,
    "",
    "Opportunity Recovery Estimate",
    ...
    "3. Consolidate duplicate data capture and create a source-of-truth policy.",
    "4. Automate only after process ownership and exception paths are clear.",
    "",
    "Recommended Next Step",
    "Book an Operational Intelligence Audit with Mindful Tech to validate the findings, quantify the highest-value workflow improvements, and design an execution plan.",
  ].join("\n");
}
